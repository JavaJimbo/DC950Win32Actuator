/* 	CRC Check.c Routines for creating and checking CRC 
 *	Used when communicating with Interface Board
 *
 ...
 *                                                                   *
\*******************************************************************/
UINT16 update_crc_16(UINT16 crc, char nextVal) {
    UINT16 tmp, shortVal;

    shortVal = 0x00ff & (UINT16) nextVal;
    tmp = crc^shortVal;
...
//	CRCcalculate():
//	This routine does a CRC for the incoming string referenced by ptrPacket.
...
UINT16 CRCcalculate(char *ptrPacket, BOOL addCRCtoPacket) {
	...
	for (i = 0; i < length; i++) {
		ch = ptrPacket[i];
		if (ch == '[') break;
		CRCresult = update_crc_16(CRCresult, ch);
	}

    if (addCRCtoPacket) {
        char strHexCRC[8];
        sprintf_s(strHexCRC, "[%04X]\r", CRCresult);
        strcat_s(ptrPacket, BUFFERSIZE, strHexCRC);
    }
    return (CRCresult);
}
